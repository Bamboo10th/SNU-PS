...
from Model.PCFN.PCFN_P import PCFN_P
from Utils.Hybrid_loss import hybrid_loss


from Data_Loader.P_DL import get_train_loaders_p
from Data_Loader.PCFN_DL_P import get_train_loaders_pcfn_p
from Data_Loader.Fusion_DL_P import get_train_loaders_fusion_p
...
def model_select_p(opt, device):
    if opt.model_id == 'FC_EF':
        ...
        model = PCFN_P(opt.out_channels, opt.out_channels).to(device)
    else:
...
def loaders_select_p(opt, dataset_id):
    if opt.model_id == 'PCFN':
        dataset_loader = get_train_loaders_pcfn_p(opt, dataset_id)
    elif opt.model_id == 'FC_EF':
        dataset_loader = get_train_loaders_fusion_p(opt, dataset_id)
    else:
