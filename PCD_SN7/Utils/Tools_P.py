...
from Utils.Model_Select_P import model_select_p, loaders_select_p, loss_select, optimizer_select, scheduler_select
from Utils.Calculation_Selcet import calculation_select
from Utils.Utils import index_calculation_f1, conf_m, save_img, Precision
...
def trainer(opt, device):
    ...
    # 参数设置
    model = model_select_p(opt, device)
    loss_fn = loss_select(opt, device)
    train_dataloader = loaders_select_p(opt, 'Train')
    val_dataloader = loaders_select_p(opt, 'Val')
    # 优化器
...
def evaler(opt, device):
    ...
    test_dataloader = loaders_select_p(opt, 'Test')
