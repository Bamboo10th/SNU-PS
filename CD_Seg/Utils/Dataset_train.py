...
# 获取所有文件
def Read_files_test(file_path, dataset_id):
...
def Read_files_train(file_path, dataset_id):
    img_name_lsit = file_name(os.path.join(file_path, dataset_id, 'Image'))
    img_path_list = []
    label_path_list = []
    for i in range(len(img_name_lsit)):
        img_path_list.append(os.path.join(file_path, dataset_id, 'Image', img_name_lsit[i]))
        label_path_list.append(os.path.join(file_path, dataset_id, 'Label', img_name_lsit[i]))
...
class TrainDataset(Dataset):
    def __init__(self, file_dir, dataset_id):
        super().__init__()
        self.img_dir, self.label_dir = Read_files_train(file_dir, dataset_id)
...
class TestDataset(Dataset):
    def __init__(self, file_dir, dataset_id):
        super().__init__()
        self.img_dir, self.label_dir = Read_files_test(file_dir, dataset_id)
