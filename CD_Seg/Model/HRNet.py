...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    """
    3X3卷积
    :param in_planes: 输入数据的通道数
    :param out_planes: 输出数据的通道数，等于卷积核个数
    :param stride: 步长，默认为1
    :param groups:
    :param dilation:(扩张)：控制kernel点（卷积核点）的间距
    :return:
    """
...
class HighResolutionNet1(nn.Module):

    def __init__(self,
                 cfg,
                 out_channels,
                 norm_layer=None):
        super(HighResolutionNet1, self).__init__()
        ...
        num_channels = self.stage4_cfg['NUM_CHANNELS']  # [48, 96, 192, 384]
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        :param num_inchannels: 由 NUM_CHANNELS 和 block.expansion 相乘得到
        :param multi_scale_output:
        :return:num_modules 个 HighResolutionModule 串联结构
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=(4 * x.size(2), 4 * x.size(3)), mode='bilinear', align_corners=True)
...
# model = HighResolutionNet(HRNET_48, out_channels=3).to(device)
