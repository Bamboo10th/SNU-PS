    ...
    def forward(self, x):
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if self.transition1[i] is not None:
                x_list.append(checkpoint(self.transition1[i], x))
            else:
                x_list.append(x)
        ...
        for i in range(self.stage3_cfg['NUM_BRANCHES']):
            if self.transition2[i] is not None:
                if i < self.stage2_cfg['NUM_BRANCHES']:
                    x_list.append(checkpoint(self.transition2[i], y_list[i]))
                else:
                    x_list.append(checkpoint(self.transition2[i], y_list[-1]))
            else:
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                if i < self.stage3_cfg['NUM_BRANCHES']:
                    x_list.append(checkpoint(self.transition3[i], y_list[i]))
                else:
                    x_list.append(checkpoint(self.transition3[i], y_list[-1]))
            else:
                x_list.append(y_list[i])
        ...
        x = F.interpolate(x, size=(4 * x0_h, 4 * x0_w), mode='bilinear', align_corners=True)

        x = checkpoint(self.last_layer, x)
...
# model = HighResolutionNet2(HRNET_48, out_channels=2).to(device)
