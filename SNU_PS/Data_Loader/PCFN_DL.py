...
def numpy_to_tensor(img_name, f_path, dataset_id):
    img1 = io.imread(os.path.join(f_path, dataset_id, 'T1', img_name))
    label1 = io.imread(os.path.join(f_path, dataset_id, 'T1_Label', img_name))
    img2 = io.imread(os.path.join(f_path, dataset_id, 'T2', img_name))
    label2 = io.imread(os.path.join(f_path, dataset_id, 'T2_Label', img_name))
    ...
    if np.max(label1) == 255:
        label1 = (label1 == 255) * 1
    if np.max(label2) == 255:
        label2 = (label2 == 255) * 1
    if np.max(label) == 255:
        label = (label == 255) * 1

    imgA = torch.from_numpy(img1.transpose((2, 0, 1)).copy()).float()
    imgB = torch.from_numpy(img2.transpose((2, 0, 1)).copy()).float()
    labelA = torch.from_numpy(label1.copy()).float()
    labelB = torch.from_numpy(label2.copy()).float()
    label = torch.from_numpy(label.copy()).float()

    sample = {'imgA': imgA, 'imgB': imgB, 'labelA': labelA,
              'labelB': labelB, 'label': label, 'path': save_path}
    return sample
...
# 加载数据
def get_train_loaders_pcfn(opt, dataset_id):
    if dataset_id == 'Train':
